...
router.post('/violations/creation', async (req, res) => {
    try {
...
// Get a single violation record by province and plate
router.get('/search/', async (req, res) => {
    try {
        const { province,plate } = req.query;
        
        if (!plate) {
            return res.status(400).json({
                success: false,
                message: 'Missing required parameter: plate'
            });
        }
        if (!province) {
            return res.status(400).json({
                success: false,
                message: 'Missing required parameter: province'
            });
        }
        
        console.log(`Fetching violation with : ${province} ${plate}`);
        
        // Call service layer method to get a single violation record
        const result = await appService.getViolationByPlate(province,plate);
        ...
        console.error('Error fetching violation by:', error);
        res.status(500).json({
            success: false,
            message: 'Failed to fetch violation data: ' + error.message
        });
