...
async function createViolation(province, licensePlate, reason, lotId, vehicleId = null) {
    ...
    try {
        ...
        console.log(`Create Violation here`);
...
// async function getViolationByPlate(licensePlate, province) {
//     let connection;
//     try {
//         connection = await oracledb.getConnection(dbConfig);
...
//             try {
//                 await connection.close();
//                 console.log('Database connection closed successfully in getViolationByPlate');
//             } catch (err) {
//                 console.error('Error closing database connection in getViolationByPlate:', err);
//             }
//         }
//     }
// }
...
async function getViolationById(ticketId) {
    ...
    try {
        ...
        if (isNaN(ticketIdInt)) {
            console.error('TicketId is not a valid number:', ticketId);
            return {
                success: false,
                message: 'Invalid ticket ID provided.'
            };
        }else{
            console.error('TicketId is a valid number:', ticketId);
        }
        const result = await connection.execute(
            query,
            {
...
// Find single violation record by Plate
async function getViolationByPlate(province, plate) {
    let connection;
    try {
        connection = await oracledb.getConnection(dbConfig);
        ...
        const query = `
            SELECT 
                ...
                v.STATUS
            FROM Violations v
            WHERE v.LICENSE_PLATE = :plate AND 
                  v.PROVINCE = :province
        `;
        ...
        if (result.rows.length === 0) {
            return {
                success: false,
                message: `No violation found with PLATE ${province} ${plate}`
            };
        }
        ...
        console.error('Error finding violation by Plate:', error);
        return {
            success: false,
            message: 'Database error while finding violation: ' + error.message
        };
