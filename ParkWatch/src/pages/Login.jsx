...
const Login = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                // Login successful
                console.log('Login successful, got user data:', data.user);

                // Debug user role and userType
                const role = data.user.role;
                const userType = data.user.userType;
                console.log(`User role: ${role}, User type: ${userType}`);

                // Store user role correctly
                let effectiveRole = role;
                
                // Some code seems to expect resident/visitor as role when it's actually stored in userType
                // This ensures we use the proper values for routing
                if (role === 'user' && userType) {
                    console.log(`User has role 'user' and type '${userType}', using type as effective role`);
                    effectiveRole = userType;
                }
                
                console.log('Storing user with effective role:', effectiveRole);
                login(effectiveRole, data.user);
                ...
                console.log('Navigating based on role and type...');
                if (role === 'admin') {
                    console.log('Navigating to admin dashboard');
                    navigate('/dashboard');
                } else if (role === 'user' && userType === 'resident') {
                    console.log('Navigating to resident dashboard');
                    navigate('/resident-dashboard');
                } else if (role === 'user' && userType === 'visitor') {
                    console.log('Navigating to visitor dashboard');
                    navigate('/visitor-dashboard');
                } else {
                    console.log('Unknown role/type combination, going to home page');
                    navigate('/');
                    toast({
